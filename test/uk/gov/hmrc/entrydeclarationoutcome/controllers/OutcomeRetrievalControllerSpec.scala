...
class OutcomeRetrievalControllerSpec
  ...
  "OutcomeRetrievalController listOutcomes" must {
    "return 200 OK with outcomes for CSP client" when {
      "the user is authenticated as a CSP client and unacknowledged outcomes are found" in {
        MockAuthService.authenticate() returns Future.successful(Some(userDetailsCSP))
        MockOutcomeXmlRetrievalService.listOutcomes(userDetailsCSP) returns Future.successful(
    ...
    "return 204 NO_CONTENT for CSPClient" when {
      "the user is authenticated as a CSPClient but no outcomes are found" in {
        MockAuthService.authenticate() returns Future.successful(Some(userDetailsCSP))
        MockOutcomeXmlRetrievalService.listOutcomes(userDetailsCSP) returns Future.successful(List.empty[OutcomeMetadata])
