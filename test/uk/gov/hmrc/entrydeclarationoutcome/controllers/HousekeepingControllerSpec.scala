...
import uk.gov.hmrc.entrydeclarationoutcome.services.MockHousekeepingService
  ...
  "HousekeepingController" when {
    ...
    "setting housekeeping state" when {
      "setting true is successful" must {
        "return 204" in {
          MockHousekeepingService.enableHousekeeping(true) returns true

          val result = controller.setStatus()(FakeRequest().withBody(houseKeepingStatusJson(true)))
      ...
      "setting false is successful" must {
        "return 204" in {
          MockHousekeepingService.enableHousekeeping(false) returns true

          val result = controller.setStatus()(FakeRequest().withBody(houseKeepingStatusJson(false)))
      ...
      "setting fails" must {
        "return 500" in {
          MockHousekeepingService.enableHousekeeping(true) returns false
